...
export function SocialMedia({
  ...
  const textColor = invert ? 'white' : 'gray.950';
  const hoverColor = invert ? 'gray.200' : 'gray.700';

  return (
    <>
      {title && <Text fontWeight={'bold'}>Follow us</Text>}

      <HStack spacing={10} color={textColor} mt={'24px'}>
        {socialMediaProfiles.map((socialMediaProfile) => (
          <Box as="li" key={socialMediaProfile.title} listStyleType="none">
            <Link
